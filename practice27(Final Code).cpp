...
void InitialiseUAREA()
{
    ...
    printf("Infosystem CVFS : UAREA gets initialised successfully\n ");
}
...
void InitialiseSuperBlock()
{
    ...
    printf("Infosystem CVFS : Super block gets initialised successfully\n ");
}
...
void CreateDILB()
{
    ...
    printf("Infosystem CVFS : DILB created successfully\n");
}

///////////////////////////////////////////////////////////////////////
//
//      Function Name:      StartAuxillaryDataInitilisation
//      Description:        It is used to call all such function which 
...
{
    strcpy(bootobj.Information,"Booting process of Infosystem CVFS is done\n");
    ...
    printf("Infosystem CVFS : Auxillary data initialised successfully\n");
}
...
{
    printf("-------------------------------------------------------\n");
    printf("--------------Infosystem CVFS Help Page----------------\n");
    printf("-------------------------------------------------------\n");
                ...
                )
{
   ...
   if(name == NULL)
   {
    ...
    //Initialise file tabe
    uareaobj.UFDT[i]->ReadOfSet = 0;
    uareaobj.UFDT[i]->WriteOfSet = 0;
...
void LsFile()
{
    ...
    printf("-------------------------------------------------------\n");
    printf("---------Infosystem CVFS File Information--------------\n");
    printf("-------------------------------------------------------\n");
...

{
    ...
    //Insufficient Space
    if((MAXFILESIZE - uareaobj.UFDT[fd]->WriteOfSet) < size)
    {
        return ERR_INSUFFICIENT_SPACE;
    }

    //Write the data into the file
    strncpy(uareaobj.UFDT[fd]->ptrinode->Buffer + uareaobj.UFDT[fd]->WriteOfSet, data, size);

    //Update the writeofset
    uareaobj.UFDT[fd]->WriteOfSet = uareaobj.UFDT[fd]->WriteOfSet + size;
...

{
    ...
    //Insufficeint Data
    if((MAXFILESIZE - uareaobj.UFDT[fd]->ReadOfSet) < size)
    {
        return ERR_INSUFFICIENT_DATA;
    }

    //Read the data
    strncpy(data,uareaobj.UFDT[fd]->ptrinode->Buffer + uareaobj.UFDT[fd]->ReadOfSet,size);

    //Update the read ofset
    uareaobj.UFDT[fd]->ReadOfSet = uareaobj.UFDT[fd]->ReadOfSet + size;
...
int main()
{
    ...
    printf("-------------------------------------------------------\n");
    printf("---------Infosystem CVFS started successfully----------\n");
    printf("-------------------------------------------------------\n");
    ...
    while(1)
    {
        ...
        if(iCount == 1)
        {
            //Infosystem CVFS : > exit
            if(strcmp("exit",Command[0]) == 0)
            {
                printf("Thank You for using Infosystem CVFS\n");
            ...
            //Infosystem CVFS : > help
            else if(strcmp("help",Command[0]) == 0)
            {
                DisplayHelp();
            }
            //Infosystem CVFS : > clear
        ...
        {
            //Infosystem CVFS : > man ls
            if(strcmp("man",Command[0]) == 0)
            {
                ManPageDisplay(Command[1]);
            }
            //Infosystem CVFS : > Unlink Demo.txt
            ...
            else if(strcmp("write",Command[0]) == 0)
            {
                ...
                if(iRet == ERR_INVALID_PARAMETER)
                {
                    printf("Error: invalid paramters\n");
                }
        ...
        {
            //Infosystem CVFS : > creat Ganesh.txt 3
            if(strcmp("creat",Command[0]) == 0)
            {
                ...
                if(iRet == ERR_INVALID_PARAMETER)
                {
                    printf("Error: Unable to create the file as paramters are invalid\n");
                ...
                {
                    printf("Error: Unable to create file because the file is already exit\n");
                }
